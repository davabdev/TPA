					...
					else if constexpr (std::is_same<T, float>() && std::is_same<RES, float>())
					{
						...
						else if (tpa::hasAVX)
						{
							...
							for (; (i + 8) < end; i += 8)
							{
								...
								_num = tpa::simd::_mm256_abs_ps(_num);
					...
					else if constexpr (std::is_same<T, double>() && std::is_same<RES, double>())
					{
						...
						else if (tpa::hasAVX)
						{
							...
							for (; (i + 4) < end; i += 4)
							{
								...
								_num = tpa::simd::_mm256_abs_pd(_num);
					...
					else if constexpr (std::is_same<T, int64_t>() && std::is_same<RES, int64_t>())
					{
						...
						else if (tpa::hasAVX2)
						{
							...
							for (; (i+4) < end; i += 4)
							{
								...
								_num = tpa::simd::_mm256_mul_epi64(_num, _mult);
